...
> **v2.4** · 端-管-云一体化智能家居物联网平台，贯通「传感器采集 → 边缘处理 → MQTT 通信 → 云平台可视化 → 远程控制」全链路。
...
- **递归多步预测**：预测 t+10 / t+20 / ... 至 t+horizon 分钟，上一步预测值影响下一步
- **动态时长**：前端可选 30/60/120/180/240 min，后端动态生成步列表（3/6/12/18/24 步）
- **即时训练**：每次点击从数据库拉取 144 条（~24h）历史数据，即训即测
- **评估指标**：MAE（平均绝对误差）+ R²（决定系数）
- **过期清理**：`predicted_at + horizon_minutes < 最新数据时间` 的记录惰性删除
...
| 模型类型 | 输入特征 | 输出 |
|------|------|------|
| `mlp_temp_hum` | `[t_sec, temperature, humidity]` | 未来 N 步温湿度（10~240min，动态） |
| `mlp_light` | `[t_sec, light]` | 未来 N 步光照（10~240min，动态） |
