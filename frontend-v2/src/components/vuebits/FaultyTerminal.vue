...
const props = defineProps({
  ...
  noiseAmp: { type: Number, default: 1 },
...
const rafRef = ref(0)
const loadAnimationStartRef = ref(0)
const timeOffsetRef = ref(Math.random() * 100)

const effectiveDpr = computed(() =>
  props.dpr !== null ? props.dpr : Math.min(window.devicePixelRatio || 1, 1)
)
...
const handleMouseMove = (e) => {
  pendingMouseEvent = e
  if (mouseRafRef) return
  mouseRafRef = requestAnimationFrame(() => {
    mouseRafRef = 0
    const ctn = containerRef.value
    if (!ctn || !pendingMouseEvent) return
    const rect = ctn.getBoundingClientRect()
    const x = (pendingMouseEvent.clientX - rect.left) / rect.width
    const y = 1 - (pendingMouseEvent.clientY - rect.top) / rect.height
    mouseRef.value = { x, y }
  })
}
...
const setup = () => {
  ...
  const renderer = new Renderer({ dpr: effectiveDpr.value })
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uTint: {
        value: new Color(tintVec.value[0], tintVec.value[1], tintVec.value[2])
      },
      uMouse: {
  ...
  if (props.mouseReact) window.addEventListener('mousemove', handleMouseMove, { passive: true })

  visibilityHandler = () => {
    if (document.hidden) {
      if (mouseRafRef) {
        cancelAnimationFrame(mouseRafRef)
        mouseRafRef = 0
      }
    }
  }
  ...
  cleanupFn = () => {
    ...
    if (props.mouseReact) window.removeEventListener('mousemove', handleMouseMove)
...
watch(
  () => [props.dither],
  () => {
    if (programRef.value) programRef.value.uniforms.uDither.value = ditherValue.value
  }
)
...
watch(
  ...
  () => {
    ...
    const c = programRef.value.uniforms.uTint.value
    c.r = rgb[0]
    c.g = rgb[1]
    c.b = rgb[2]
  }
